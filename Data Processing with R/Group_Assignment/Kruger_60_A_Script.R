...
weather_clean_r1_1 <- subset(weather_clean, select = -c(Date, RISK_MM,Location, 
                                                        Season, WindDir3pm,
                                                        WindSpeed9am, 
                                                        WindDir9am))
weather_clean_r1_1$RainTomorrow <- relevel(factor(weather_clean_r1_1$RainTomorrow),
                                           ref = "Yes")
set.seed(1)
trn_index = createDataPartition(y = weather_clean_r1_1$RainTomorrow, p = 0.70, 
                                list = FALSE)
trn_weather = weather_clean_r1[trn_index, ]
tst_weather = weather_clean_r1[-trn_index, ]
...
trn_index <- createDataPartition(weather_frac$RainTomorrow, p = 0.7, 
                                 list = FALSE)
...
knn_weather <- train(RainTomorrow ~. -Date, method = "knn", data = trn_weather,
                     ...
                     na.action = na.omit)
knn_weather
# k = 7 proves to be the most accurate (0.872)

# Shelly van Erp----------------------------------------------------------------
## Modelling Research Question 2 -----------------------------------------------


## Sample Data and Train-Test Split
